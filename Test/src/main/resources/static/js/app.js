...
async function loadStats() {
    try {
        ...
        // Update hero stats
        const heroTotal = document.getElementById('hero-total-coupons');
        if (heroTotal) {
            heroTotal.textContent = data.activeCoupons || 0;
        }
    } catch (error) {
...
function initializeDemo() {
    demoMode = true;
    loadDemoCart();
    showToast('Demo mode activated! Try applying coupons to see the magic.', 'info');
}

function loadDemoCart() {
    // Demo cart is already loaded in HTML
    showToast('Sample cart loaded with 3 items worth $440', 'success');
}

function findDemoCoupons() {
    // Animate the coupon cards
    const coupons = document.querySelectorAll('.demo-coupon');
    coupons.forEach((coupon, index) => {
        setTimeout(() => {
            coupon.style.animation = 'slideInUp 0.5s ease-out';
            coupon.style.transform = 'scale(1.02)';
            setTimeout(() => {
    ...
    showToast('Found 2 applicable coupons! Choose the best deal.', 'success');
}

function applyDemoCoupon(type) {
    const resultsSection = document.getElementById('demo-results');
    const originalEl = document.getElementById('demo-original');
    const savingsEl = document.getElementById('demo-savings');
    ...
    if (type === 'cart-wise') {
        savings = 44.00;
        final = 396.00;
        showToast('Cart-wise coupon applied! 10% discount on your order.', 'success');
    } else if (type === 'bxgy') {
        savings = 50.00;
        final = 390.00;
        showToast('BxGy coupon applied! Free USB Cable added to your cart.', 'success');
    }

    // Update values
    originalEl.textContent = '$440.00';
    ...
    // Disable coupon buttons
    const buttons = document.querySelectorAll('.demo-coupon button');
    buttons.forEach(btn => {
        btn.disabled = true;
